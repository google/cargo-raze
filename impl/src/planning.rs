  ...
  fn produce_targets(
    ...
    for target in package.targets.iter() {
      ...
      let local_root = match (settings.genmode.clone(), is_git) {
        ...
        (GenMode::Remote, true) => try!(package_git_root(&manifest_pathbuf))
          .to_str()
          .unwrap()
          .to_owned(),
      ...
      let mut local_path_str = target.src_path.clone().split_off(local_root.len() + 1);
...
fn package_git_root(manifest: &PathBuf) -> CargoResult<PathBuf> {
  ...
  Err(CargoError::from(format!(
    "Unable to locate git repository root for manifest {:?}",
    manifest
  )))
}
